...
describe("full 6 digit expression", () => {
  ...
  it.each(cases)("%s", (_title, cron, timezone1, timezone2, expected) => {
    expect(cronTzConverter(cron, timezone1, timezone2)).toEqual(expected);
  });
});

describe("5 digit expression, no seconds", () => {
  const cases: [string, string, string, string, string][] = [
    [
  ...
  it.each(cases)("%s", (_title, cron, timezone1, timezone2, expected) => {
    expect(cronTzConverter(cron, timezone1, timezone2, false)).toEqual(
      expected,
    );
  });
});
